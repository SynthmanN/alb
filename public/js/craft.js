...
function applyManualPrices(data) {
  ...
  for (const r of d.recipe) {
    ...
    if ((refineChanged || ownComp) && r.refineOption) {
      ...
      if (buy === null || alt < buy) { r.materialSource = 'refine'; r.cheapestPrice = alt; r.cheapestCity = r.refineOption.city; r.priceSource = 'refine'; r.buyPrice = buy; }
    ...
    if (own !== undefined && p !== r.cheapestPrice) { r.cheapestPrice = p; r.manualPrice = true; }
  }
    ...
    <ol class="craft-flow">
      ...
      <li class="craft-step">
        ...
        <div class="step-body">
          ...
          <div class="craft-summary">
      <div class="craft-summary-row"><span>Продажа в Buy Order: лучшая цена (мгновенно, в чужой ордер на покупку)</span><span>${data.bestSell && !data.bestSell.manual ? `${data.bestSell.blackMarket ? '⚫ ' : ''}${data.bestSell.city}: ${data.bestSell.price.toLocaleString('ru-RU')}` : data.bestSell ? `своя цена: ${data.bestSell.price.toLocaleString('ru-RU')}` : 'нет данных'} <input id="manual-sell-price" class="manual-price ${manualSellPrice !== null ? 'is-manual' : ''}" type="number" min="0" step="1" placeholder="своя цена" value="${manualSellPrice !== null ? manualSellPrice : ''}" title="Вписал цену, по которой реально продаёшь, — профит пересчитается сразу" /></span></div>
...
function refineSourceHtml(r) {
  ...
  const parts = o.components.map((c) => `${c.count}× ${itemLabel(c.id)}`).join(' + ');
  const buy = r.buyPrice ? `; готовый на рынке — ${fmtNum(r.buyPrice)}` : '';
  return `<span class="refine-source" title="Сырьё и полуфабрикат предыдущего тира по рыночным ценам, ставка возврата при переработке ${(o.rate * 100).toFixed(1)}%${buy}">♻ выгоднее переработать в ${o.city}: ${fmtNum(r.cheapestPrice)}</span><br><small>${parts}</small>`;
}
...
function scoreboardHtml(data) {
  const money = (n) => (n === null || n === undefined ? '—' : Math.round(n).toLocaleString('ru-RU'));
  ...
  return `
    <div class="craft-scoreboard">
      <div class="sb-cell sb-cost"><span class="sb-label">Нужно денег на весь цикл</span><b class="sb-value">${money(data.totalCost)}</b><small>${money(data.effectiveCostPerUnit)} за штуку · ${data.quantity.toLocaleString('ru-RU')} шт${cycle !== null ? ` · цикл ${fmtDays(cycle)}` : ''}</small></div>
...
function renderMarginScan(data) {
  ...
  const rows = data.results.map((r) => {
    ...
    return `
      <tr>
        ...
        <td data-sort-value="${r.dailyVolume}" title="${r.liquidityRatio ? `Ликвидность относительно цены: ${r.liquidityRatio.toFixed(1)}× от медианы комбинаций той же ценовой полосы (${fmtNum(r.liquidityBandMedian, 1)}/день)` : 'Мало комбинаций той же цены для сравнения — ликвидность не оценивалась'}">${volumeCell(r, patient && data.liquidity !== 'best')}</td>
  ...
  marginEl.result.innerHTML = `
    <p class="calc-note">Просмотрено комбинаций: ${fmtNum(data.scanned)}. ${data.mode === 'patient' ? 'Терпеливый режим' : 'Мгновенный режим'}: ${sellNote}. Скан ищет, что выгодно крафтить: для каждого предмета остаётся ОДНА комбинация (тир/зачарование/качество) — самая большая маржа за штуку среди прибыльных и достаточно ликвидных для своей цены (дорогой гир торгуется реже — он сравнивается с гиром той же ценовой полосы; «мусор» с раздутой маржой и мёртвым оборотом отсеивается). Список открывается отсортированным по «Марже/шт» по убыванию; клик по заголовку — другая сортировка. Возврат, количество и точные цены — в калькуляторе («в калькулятор» у строки). Способ зачарования: ${data.enchantMode === 'after' ? 'после крафта рунами' : 'крафт из зачарованного сырья'}; проверенный диапазон зачарования: ${data.enchantRange}. ${data.blackMarket ? `Чёрный Рынок учтён (налог ${(data.bmTaxRate * 100).toFixed(1)}%, помечен ⚫). ` : ''}Возврат при крафте: ${(data.rrrOptions.gearRate * 100).toFixed(1)}%${data.rrrOptions.gearRrrCustom !== null ? ' (своя ставка)' : ''}. Материал берётся дешевле из двух путей: купить готовым или переработать самому (♻) из сырья и полуфабриката предыдущего тира с возвратом при переработке ${(data.refineRate * 100).toFixed(1)}%. ${jugNote}</p>
    <div class="table-scroll"><table class="scan-table">
      <thead><tr><th>Предмет</th><th>Качество</th><th>Себестоимость</th><th>${patient ? 'Ср. цена продажи' : 'Buy Order'}</th><th>Оборот/день (рынок)</th><th title="Прибыль с одной штуки после налога и сбора, в серебре (и в % от себестоимости)">Маржа/шт</th><th title="Маржа/шт × оборот/день: масштаб рынка в серебре">Профит рынка/день</th><th title="По слабому звену — предмету и его сырью">Доверие</th><th>Свежесть</th><th></th></tr></thead>
      <tbody>${rows}</tbody>
    </table></div>`;
  tableSortStates['margin-scan'] = { label: 'Маржа/шт', dir: 'desc' };     // после каждого скана — сразу по марже за штуку
