...
export function StackView() {
  ...
  return html`<section class="panel" id="panel-calc">
    <div class="stackhead">
      ...
      <div class="stackactions"><button class="btn" type="button" id="stack-add" onClick=${() => setAdding(!adding)}><${Icon} d=${ICONS.plus} />Добавить позицию</button><${FreshnessButton} ids=${collectAllIds(data.results, (d) => itemLabel(d.itemId))} onRefreshed=${() => invalidateDef(stackDef)} /><button class="btn ghost" type="button" id="stack-exit" onClick=${leaveStack}>Выйти из стека</button></div>
    </div>
