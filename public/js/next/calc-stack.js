...
function SalesTable({ data }) {
  ...
  return html`<div class="card tw" id="stack-sales"><table><thead><tr><th>Позиция</th><th>Кол-во</th><th>Вложения / шт</th><th>Профит / шт</th><th>Профит всего</th><th>Срок продажи</th><th>Очки</th></tr></thead>
    ...
    <div class="statusline">Цена продажи — из плана или своя (в карточке позиции); срок — по плану продажи калькулятора. Клик по строке открывает позицию подробно.</div></div>`;
}
...
export function StackView() {
  ...
  return html`<section class="panel" id="panel-calc">
    ...
    ${items.length === 0 ? html`<div class="card empty">Стек пуст. Добавь позицию или открой активные позиции из крафт-листа.</div>` : html`
      ...
      ${c.sub === 'sell' ? html`<${SalesTable} data=${data} />` : html`<${StackShopping} data=${data} />`}`}
  </section>`;
}
