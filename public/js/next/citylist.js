...
export function CityPriceList({ resKey, list, own, onSet, fee = SETUP_FEE, label = 'Все города' }) {
  ...
  const rows = cityRows(list, own, activeCities(s), fee);
  ...
  return html`<details class="cityprices" data-res=${resKey}>
    ...
    <div class="cp-list" role="table" aria-label=${`Цены по городам: ${resKey}`}>
      ${rows.map((r) => html`<div class=${`cp-row ${r.isBest ? 'is-best' : ''}`} role="row" key=${r.city}>
        <${CityPill} name=${r.city} />
      ...
      <p class="cp-note">Рыночные цены без комиссии; для сравнения к ним добавляется 2.5% за свой Buy Order, а своя цена считается как есть.</p>
    </div></details>`;
}
