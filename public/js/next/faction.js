      ...
      <div class="card"><div class="tw"><table id="plan-table">
        ...
        <tbody>${visible.map((x) => {
          ...
          return html`<${Fragment} key=${c.key}><tr class=${on ? 'on-plan' : 'off'} data-row=${c.key}>
            ...
            <td>${fmt(c.grossSale)}${r.sale && r.sale.orderOnly ? html` <span class="fp-warn" title="Сделок за период нет — цена самого дорогого свежего ордера на продажу; оборот неизвестен">по ордеру</span>` : null}${r.sale && r.sale.manual ? html` <span class="fp-warn" title="Вписано вручную — недостоверная цена">⚠</span>` : null}</td>
