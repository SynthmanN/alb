    ...
    <aside class="drawer wide" id="drawer-list" aria-label="Крафт-лист">
      ...
      <div class="scroll">
        ${items.length === 0 ? html`<div class="empty">Лист пуст.<br />Добавляй позиции кнопкой «+» в скане, из калькулятора или целым планом.</div>` : html`
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn" type="button" onClick=${copyNames}>Скопировать список</button><${FreshnessButton} ids=${collectAllIds(data.results, (d) => itemLabel(d.itemId), data.pairs)} onRefreshed=${() => invalidateDef(listDef)} /><button class="btn ghost" type="button" onClick=${clearList}>Очистить</button></div>`}
      </div></aside></${Fragment}>`;
}
