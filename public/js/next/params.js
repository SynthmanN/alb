...
export function ParamsBar() {
  ...
  return html`<section class="card pbar" aria-label="Параметры расчёта">
    ...
    ${more ? html`<div class="adv" id="adv">
      ...
      <${Switch} checked=${s.buyReady} onChange=${(v) => settings.set({ buyReady: v })} title="Ещё один вариант зачарки: купить уже готовый предмет .1/.2 на аукционе и докрутить только оставшимися шагами">Покупка готового уровня</${Switch}>
