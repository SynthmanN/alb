...
describe('computeRow', () => {
  it('прямой путь и «после крафта»: после выбирается только если дешевле прямого на 5%+', () => {
    const cheap = computeRow(row(4, 2), ctx());                       // после = 1000 + 2 × 96 × 10 = 2920 ≤ 4750 → после
    expect(cheap.path).toBe('after');
    expect(cheap.cost).toBe(2920);
    const dear = computeRow(row(4, 2, { capeDirect: { id: 'X', label: 'Плащ', price: 3000 } }), ctx());   // 2920 > 3000 × 0.95 = 2850 → прямой
    expect(dear).toMatchObject({ path: 'direct', cost: 3000 });
  });
