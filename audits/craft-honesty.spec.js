...
const calcPayload = (qty) => ({
  ...
  patientSell: {
    ...
    cities: [],
    plan: { bestPrice: 8000, avgPrice: 7500, overpayPct: 0, totalDays: 8, excluded: [{ city: 'Thetford', reason: 'вне допуска' }],
...
test('цикл игрока: числа итога совпадают с независимым пересчётом при любых правках плана', async ({ page }) => {
  await page.route('**/api/craft-margin-opportunities*', (route) => route.fulfill({ json: { enchantMode: 'direct', liquidity: 'sum', days: 7, marketShare: 0.5, taxRate: TAX, premiumPrice: 28000000, scanned: 1, results: [
    { itemId: 'T4_MAIN_SWORD', enchant: 0, quality: 1, cost: COST, avgSellPrice: 7500, dailyVolume: 70, yourDailyVolume: 35, marketDailyVolume: 100, sellCities: ['Lymhurst', 'Martlock'], profitPerUnit: 3000, profitPct: 80, dailyProfit: 105000, premiumDays: 266, score: 100 }] } }));
