...
// AODP хранит ПОЧАСОВУЮ историю только за ~7 дней; дни 8–10 берутся дневными точками (time-scale=24) — раз в 6 часов: они почти не меняются.
// Диапазон дневного запроса не пересекается с почасовым, поэтому одни и те же сделки не считаются дважды.
const DAILY_EVERY_MS = 6 * 60 * 60 * 1000;
...
// Дневные точки для дней 8–10 (за пределами почасовой истории AODP). fetchHistoryDaily(chunk, cities) -> записи AODP с дневными точками.
async function crawlHistoryDailyOnce({ db, ids, jobs, fetchHistoryDaily, log = () => {}, now = Date.now }) {
  let failed = 0;
  for (const job of asJobs({ jobs, ids })) for (const chunk of chunkList(job.ids, HISTORY_CHUNK)) {
    try {
      upsertHistoryBatch(db, await fetchHistoryDaily(chunk, job.cities), now());
    } catch (err) {
      failed++;
      log(`кувшин: не удалось обновить дневную историю для чанка: ${err.message}`);
    }
  }
  if (failed === 0) setMeta(db, 'lastDailyPass', now());
  return { failed };
}

// Бесконечный цикл: цены → (если пора) история → «полный проход» → короткая пауза. stop() останавливает после текущего чанка.
function startJugCrawler({ db, ids, jobs, fetchPrices, fetchHistory, fetchHistoryDaily = null, log = console.log, now = Date.now, pauseMs = 5000, cycleMs = 0, sleep = (ms) => new Promise((r) => setTimeout(r, ms)) }) {
  let stopped = false;
  const loop = (async () => {
    while (!stopped) {
      const started = now();
      try {
        ...
        if (!stopped && now() - lastHistory >= HISTORY_EVERY_MS) {
          ...
          const lastDaily = Number(getMeta(db, 'lastDailyPass')) || 0;
          if (fetchHistoryDaily && !stopped && now() - lastDaily >= DAILY_EVERY_MS) await crawlHistoryDailyOnce({ db, ids, jobs, fetchHistoryDaily, log, now });
...
module.exports = { DAILY_EVERY_MS, crawlHistoryDailyOnce, CYCLE_MS, chunkList, crawlPricesOnce, crawlHistoryOnce, startJugCrawler, PRICE_CHUNK, HISTORY_CHUNK, HISTORY_EVERY_MS };
