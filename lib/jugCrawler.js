// Фоновый краулер «кувшина»: обходит весь каталог порциями и кладёт цены и историю сделок в локальную базу.
// Темп задаёт общий регулятор бюджета AODP (fetchChunk внутри уже ждёт своей очереди), поэтому сам краулер не спит между чанками.
// Цены обновляются на каждом круге, история — реже (она меняется медленно, а запросов на неё в разы больше).
const { upsertPriceSnapshots, upsertHistoryBatch, setMeta, getMeta } = require('./jugStore');

const PRICE_CHUNK = 120;   // id в одном запросе цен (упирается в лимит URL AODP — 4096 символов)
const HISTORY_CHUNK = 25;  // id в одном запросе истории
const HISTORY_EVERY_MS = 15 * 60 * 1000;

function chunkList(list, size) {
  const out = [];
  for (let i = 0; i < list.length; i += size) out.push(list.slice(i, i + size));
  return out;
}

// Один проход по ценам. fetchPrices(ids) -> записи AODP. Ошибка чанка (429 и т.п.) не роняет проход: чанк пропускается и логируется.
async function crawlPricesOnce({ db, ids, fetchPrices, log = () => {}, now = Date.now }) {
  let failed = 0;
  for (const chunk of chunkList(ids, PRICE_CHUNK)) {
    try {
      upsertPriceSnapshots(db, await fetchPrices(chunk), now());
    } catch (err) {
      failed++;
      log(`кувшин: не удалось обновить цены для чанка: ${err.message}`);
    }
  }
  setMeta(db, 'lastPricePass', now());
  return { failed };
}

async function crawlHistoryOnce({ db, ids, fetchHistory, log = () => {}, now = Date.now }) {
  let failed = 0;
  for (const chunk of chunkList(ids, HISTORY_CHUNK)) {
    try {
      upsertHistoryBatch(db, await fetchHistory(chunk), now());
    } catch (err) {
      failed++;
      log(`кувшин: не удалось обновить историю для чанка: ${err.message}`);
    }
  }
  setMeta(db, 'lastHistoryPass', now());
  return { failed };
}

// Бесконечный цикл: цены → (если пора) история → «полный проход» → короткая пауза. stop() останавливает после текущего чанка.
function startJugCrawler({ db, ids, fetchPrices, fetchHistory, log = console.log, now = Date.now, pauseMs = 5000, sleep = (ms) => new Promise((r) => setTimeout(r, ms)) }) {
  let stopped = false;
  const loop = (async () => {
    while (!stopped) {
      try {
        const prices = await crawlPricesOnce({ db, ids, fetchPrices, log, now });
        const lastHistory = Number(getMeta(db, 'lastHistoryPass')) || 0;
        if (!stopped && now() - lastHistory >= HISTORY_EVERY_MS) {
          const history = await crawlHistoryOnce({ db, ids, fetchHistory, log, now });
          if (prices.failed === 0 && history.failed === 0) {
            setMeta(db, 'lastFullPass', now());
            log(`кувшин: полный проход каталога завершён ${new Date(now()).toISOString()}`);
          }
        }
      } catch (err) {
        log(`кувшин: сбой прохода: ${err.message}`);
      }
      if (!stopped) await sleep(pauseMs);
    }
  })();
  return { stop: () => { stopped = true; }, done: loop };
}

module.exports = { chunkList, crawlPricesOnce, crawlHistoryOnce, startJugCrawler, PRICE_CHUNK, HISTORY_CHUNK, HISTORY_EVERY_MS };
