...
// сайт (то же, что отдаёт REST AODP — CITY_DISPLAY в server.js). Подтверждено сверкой с реальными ценами AODP по каждому
// коду (одинаковая цена одного и того же предмета в NATS и в REST для этого города) — см. README. Города, для которых
// сопоставление ещё не подтверждено, в таблице нет: такой LocationId просто пропускается (лучше не писать в кувшин,
// чем подписать чужим городом) — их можно дописать сюда, когда появится точное подтверждение.
