...
const scanPayload = (over = {}) => ({
  mode: 'patient', days: 7, capital: 500000, minDays: 1, taxRate: 0.08, setupFeeRate: 0.025, premiumPrice: 28000000, scanned: 35,
  ...
  results: [
    { kind: 'material', itemId: 'T4_METALBAR', type: 'ORE', tier: 4, cost: 204, avgSellPrice: 260, sellCities: ['Martlock'], dailyVolume: 378799, profitPerUnit: 50, profitPct: 24.5,
      quantity: 2450, positionCost: 499800, batchProfit: 122500, daysToAcquire: 0.0, daysToSell: 0.003, cycleDays: 0.003, effectiveDays: 1, cappedByMinDays: true, dailyProfit: 50000, premiumDays: 560,
    ...
    { kind: 'material', itemId: 'T5_PLANKS', type: 'WOOD', tier: 5, cost: 900, avgSellPrice: 1300, sellCities: ['Fort Sterling'], dailyVolume: 800, profitPerUnit: 250, profitPct: 28,
      quantity: 555, positionCost: 499500, batchProfit: 138750, daysToAcquire: 3, daysToSell: 1.4, cycleDays: 4.4, effectiveDays: 4.4, cappedByMinDays: false, dailyProfit: 56800, premiumDays: 490,
      rawRrr: 0.153, prevRrr: 0.153, cityBonus: false, tradeHours: 4, confidence: 4 / 24, freshMinutes: 30, rankScore: 56000 },
  ],
  ...over,
});

test('скан рефайна: живёт на странице «Рефайн», параметры (капитал, минимум дней, возврат) уходят в запрос, доли рынка нет', async ({ page }) => {
  let scanQuery = null;
  ...
  await page.locator('#refine-scan-capital').fill('2000000');
  ...
  expect(scanQuery.get('capital')).toBe('2000000');
  ...
  await expect(page.locator('#refine-scan-result')).toContainText('Капитал на позицию');
